...
void Dijkstra::runDijkstra(){
	...
	distances[start] = 0; //starting point =0
	visited[start] = true; //visited is true for starting point
		for(int i = 0; i < numOfCities; i++){
			if (i != start) {
				...
				prev[i] = -1; //(NULL) no previous
			}
		}
		for(int j = 0; j < numOfCities; j++){ //Prints each matrix when we update it (total of 8)
			setDistances(minDistance());
			printInfoSoFar(); //Prints matrix so far
		}
}
...
//latest visited vertex's index number
	for(int i = 0; i < numOfCities; i++){
		if(matrixGraph[latestVert][i] + distances[latestVert] < distances[i]){
...
//then printed out the cities in reverse order.
	int *temppath = new int[numOfCities];
		int count = 0;
		temppath[count] = end;
		int prevnode = prev[end];
		count++;
		while (prevnode != start){
			temppath[count] = prevnode;
			prevnode = prev[prevnode];
			count++;
		}
		temppath[count] = start;
		for(int i = count; i >= 0; i--) {
			cout << Cities[temppath[i]] << "("<< temppath[i]<<")->"; //Prints shortest path
		}
		cout << endl;
}
/************************************************************/
